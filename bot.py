...
RADIUS_RATIO = 0.06   # rounded corner = 6% of shortest side
...
def create_mask(w: int, h: int, radius: int) -> str:
    """Create a B&W rounded-rectangle mask PNG and return its path."""
...
def run_ffmpeg(*args):
    """Run ffmpeg, raise on failure, return stdout."""
    cmd = ["ffmpeg", "-y", *args]
...
def process_video(input_path: str, out_w: int, out_h: int) -> str:
    """
    1. Crop to fill out_w×out_h (centre crop, no stretching).
    2. Apply black rounded corners via multiply blend with a mask PNG.
    ...
    # ── Step 2: build mask & apply rounded corners ────────────────────────────
    radius = int(min(out_w, out_h) * RADIUS_RATIO)
    mask_path = create_mask(out_w, out_h, radius)

    output_path = tmp("mp4")
    # multiply blend: white mask areas keep original, black areas → black corners
    run_ffmpeg(
        "-i", cropped,
        "-i", mask_path,
        "-filter_complex",
...
async def handle_video(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ctx.user_data["pending_file_type"] = "video"

    # Build ratio keyboard
    keyboard = [
...
async def handle_ratio_choice(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data  # e.g. "ratio:1080x1080"
    ...
    await query.edit_message_text(f"⏳ Processing *{label}* ({out_w}×{out_h})…\nThis may take a moment.", parse_mode="Markdown")
    ...
    try:
        # Download video
        file = await query.get_bot().get_file(file_id)
        await file.download_to_drive(input_path)

        # Process in thread so we don't block the event loop
